...
def ram(pretrained='', pretrained_condition='', **kwargs):
    # Skip weight initialization — checkpoint will overwrite all weights anyway.
    # trunc_normal_ on CPU is slow on ROCm (~42ms per call, 171 calls = ~7s wasted).
    # Skip all weight initialization — checkpoint overwrites everything anyway.
    # On ROCm, random tensor ops (normal_, uniform_, erfinv_) are slow on CPU
    # (~7ms each vs ~0.1ms on x86), so skipping ~1500 init calls saves ~10s.
    import torch.nn as _nn
    import timm.layers as _timm_layers
    from .swin_transformer_lora import SwinTransformer as _Swin
    from .bert_lora import BertPreTrainedModel as _Bert
    _orig_swin_init = _Swin._init_weights
    _orig_bert_init = _Bert._init_weights
    _orig_linear_reset = _nn.Linear.reset_parameters
    _orig_embedding_reset = _nn.Embedding.reset_parameters
    _orig_trunc_normal = _timm_layers.trunc_normal_
    _Swin._init_weights = lambda self, m: None
    _Bert._init_weights = lambda self, m: None
    _nn.Linear.reset_parameters = lambda self: None
    _nn.Embedding.reset_parameters = lambda self: None
    _timm_layers.trunc_normal_ = lambda t, **kw: t
    # also patch the module-level reference in swin_transformer_lora
    from . import swin_transformer_lora as _swin_mod
    _orig_swin_trunc = _swin_mod.trunc_normal_
    _swin_mod.trunc_normal_ = lambda t, **kw: t
    model = RAMLora(**kwargs)
    _Swin._init_weights = _orig_swin_init
    _Bert._init_weights = _orig_bert_init
    _nn.Linear.reset_parameters = _orig_linear_reset
    _nn.Embedding.reset_parameters = _orig_embedding_reset
    _timm_layers.trunc_normal_ = _orig_trunc_normal
    _swin_mod.trunc_normal_ = _orig_swin_trunc
