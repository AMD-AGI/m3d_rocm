...
if __name__ == "__main__":
    ...
    # Warmup: trigger ROCm/CUDA kernel compilation before processing real images.
    # Without this, the first image absorbs several seconds of JIT overhead.
    print('Warming up GPU kernels...')
    _dummy = torch.zeros(1, 3, args.process_size, args.process_size, device='cuda', dtype=weight_dtype)
    with torch.no_grad():
        model(_dummy * 2 - 1, prompt='warmup')
    ...
    for image_name in image_names:
        ...
        rscale = args.upscale
        target_size = args.process_size // rscale
        input_image = input_image.resize((target_size, target_size), Image.LANCZOS)
        input_image = input_image.resize((target_size * rscale, target_size * rscale), Image.LANCZOS)
